...
pTurnSpeed = GetFloatParam("turnspeed", 4)
...
function robotUpdate(dt)
	...
	if robot.enabled and hover.contact > 0 then
		...
		if vol > 0 then
			PlayLoop(walkLoop2, robot.transform.pos, vol)
		end
		...
		if distScale > 0 then
			...
			if VecDot(d, toPlayer) > 0.2 or distToPlayer < 0.1 then
				...
				if not hit or distToPlayer < 0.2 then
					SetPlayerHealth(GetPlayerHealth() - 0.01 * weapon.strength)
					--SetJointMotor(saber, -15)
					ShakeCamera(rnd(0.3, 0.9))
	...
	if config.triggerAlarmWhenSeen then
		...
		if GetBool("level.alarm") then
			...
			if playerVisible and IsPointAffectedByLight(head.eye, pp) then
				...
				PlayLoop(chargeLoop, robot.transform.pos, 2)
...
function init()
	...
	headLoop = LoadLoop("MOD/sounds/enemies/bacteria/hunt0.ogg", 25.0)
	turnLoop = LoadLoop("MOD/main/snd/villager/m3.ogg", 7.0)
	walkLoop = LoadLoop("robot/walk-loop.ogg", 7.0)
	walkLoop2 = LoadLoop("MOD/sounds/enemies/bacteria/alien.ogg", 5.0)
	rollLoop = LoadSound("MOD/sounds/enemies/bacteria/hunt0.ogg", 25.0)
	chargeLoop = LoadSound("MOD/sounds/enemies/bacteria/hunt0.ogg", 25.0)
	alertSound = LoadSound("MOD/sounds/enemies/bacteria/hunt0.ogg", 25.0)
	huntSound = LoadSound("MOD/sounds/enemies/bacteria/hunt0.ogg", 25.0)
	idleSound = LoadSound("MOD/sounds/enemies/bacteria/hunt0.ogg", 25.0)
	fireLoop = LoadLoop("tools/blowtorch-loop.ogg")
	disableSound = LoadSound("robot/disable0.ogg")
	crush = LoadSound("MOD/sounds/enemies/bacteria/bite1.ogg", 9.0)
	crush2 = LoadSound("MOD/sounds/enemies/bacteria/bite.ogg", 9.0)
	insound = LoadSound("MOD/main/snd/in01.ogg", 9.0)
	swing = LoadSound("MOD/main/snd/swng07.ogg", 9.0)
    fdeath = LoadSound("MOD/sounds/enemies/bacteria/ldeath0.ogg", 9.0)
    pain = LoadSound("MOD/sounds/enemies/bacteria/pain0.ogg", 9.0)
	pain2 = LoadSound("MOD/sounds/enemies/bacteria/bdie.ogg", 9.0)
...
function update(dt)
	...
	if robot.health <= 0.0 then
	    ...
	    local tipPos = TransformToParentPoint(robot.transform, Vec(rnd(-0.3,0.3), rnd(1.3,2.3), rnd(-0.3,0.3)))
		...
		PlaySound(pain2, robot.bodyCenter, 0.65, false)
		Paint(tipPos, 5, "explosion", 0.2)
	end
	...
	if state.id == "investigate" then
		...
		elseif state.nextAction == "done" then
			PlaySound(idleSound, robot.bodyCenter, 2.0, false)
			stackPop()
		end	
	end
	...
	if config.huntPlayer and not stackHas("hunt") then
		if config.canSeePlayer and head.canSeePlayer or robot.canSensePlayer then
			stackClear()
			PlaySound(huntSound, robot.bodyCenter, 1.8, false)
			stackPush("hunt")
		end
	end
...
function hitByExplosion(strength, pos)
	...
	local tipPos = TransformToParentPoint(robot.transform, Vec(rnd(-0.3,0.3), rnd(1.3,2.3), rnd(-0.3,0.3)))
	local tipPos2 = TransformToParentPoint(robot.transform, Vec(rnd(-0.6,0.6), rnd(0.0,2.3), rnd(-0.6,0.6)))	
	...
	if strength > 0.99 then
		...
		local damage = f * 110.0
		robot.health = robot.health - f * 110.0
		...
		for i=1, #robot.allBodies do
			...
			if velAdd > 0 then
				...
				SetBodyVelocity(b, v)
				PlaySound(pain, robot.bodyCenter, 0.6, false)
				Paint(tipPos, 5, "explosion", 0.1)
				
				ParticleGravity(-8)
				ParticleRadius(0.07, 0)
                ...
                for i=1,70 do 					
					SpawnParticle(tipPos, Vec(rnd(-5,5), rnd(-5,7), rnd(-5,5)), 5)
			    end
	            ...
	            ParticleRadius(0.4, 3)
	            ParticleStretch(10)
	            ParticleAlpha(1, 0)
	            ParticleDrag(1)
	            ParticleCollide(0)
	            ParticleGravity(-5)
			    ParticleRotation(10, 0)

				for i=1,10 do 
					Paint(tipPos2, rnd(1,3.5), "explosion", 0.1)
			    end							
		...
		if hit then
			for i=1, #robot.allShapes do
				if robot.allShapes[i] == shape then
					...
					robot.stunned = robot.stunned + 0.08
                    playVoice(pain, robot.bodyCenter, 0.5, false)
                    ...
                    for i=1,rnd(60,90) do 					
					    SpawnParticle(tipPos, Vec(rnd(-4,4), rnd(-3,5), rnd(-4,4)), 5)
					end
                    ...
                    for i=1,50 do 					
					        SpawnParticle(tipPos, Vec(rnd(-7,7), rnd(-4,9), rnd(-7,7)), 1.5)
					end					
